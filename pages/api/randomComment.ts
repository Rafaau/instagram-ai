  ...
  res: NextApiResponse
    ...
    try {
        if (count <= Number(process.env.DAILY_REQUESTS_LIMIT)) {
            const prompts = gender == 'female' ? GirlCommentPrompts : ManCommentPrompts
            ...
            res.status(200).json({ content: result.data.choices[0].text })
        } else {
            const placeholders = gender == 'female' ? GirlCommentPlaceholders : ManCommentPlaceholders
            const random = Math.floor(Math.random() * placeholders.length)
            res.status(200).json({ content: placeholders[random] })
        }
    } catch (e) {
        console.log(e)
